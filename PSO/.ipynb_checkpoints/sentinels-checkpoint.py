...
from sentinel_config import num_particles, num_iterations, w, c1, c2
...
def create_or_get_run(run_name="QPSO_Run_1", algorithm="QPSO", levy_flight=True):
    """Inserts (or retrieves) a row from 'runs' table and returns run_id."""
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()

    cursor.execute("""
        INSERT INTO runs (run_name, algorithm, levy_flight, num_particles, num_iterations)
        VALUES (?, ?, ?, ?, ?)
        ON CONFLICT(run_name) DO NOTHING
    """, (run_name, algorithm, int(levy_flight), num_particles, num_iterations))
    conn.commit()
...
def run_pso(model, distance_metric="vector", db_file=DB_FILE):
    # 1) Ensure DB schema
    init_db()
    run_id = create_or_get_run(run_name="QPSO_Run_1", algorithm="QPSO", levy_flight=True)
    ...
    iteration_end = iteration_start + num_iterations  
